...
class SetupService(
    ...
    private val bannedWordRegex =
        "(개발자|개발진|관리자|관리진|운영자|운영진|공지|알림|시스템|admin|user|끄투리오|끄투코리아|horyu|호류|국립국어원|쪼리핑|모레미|모래미|백지현|류현오|분홍꽃|포카르|드라군|ds|네코p|둔둔님|utella ?pluto|은둔자|김도훈|테드창)".toRegex()
    ...
    fun setupNick(request: HttpServletRequest, session: HttpSession, nick: String): ActionResult {
        ...
        if (nick.replace(" ", "").toLowerCase().contains(bannedWordRegex)) {
